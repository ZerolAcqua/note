...
与此同时，数字摄影结合互联网，使得照片得以实现前所未有的大规模共享。例如，在 Google 图片搜索中使用“Notre Dame AND paris”可以返回超过 15,000 张照片，从无数的视点、细节层次、光照条件、季节、十年等方面捕捉了这个场景。不幸的是，共享照片的激增速度已经超过了浏览这类收藏的技术，因为[谷歌](www.google.com)和 [Flickr](www.flickr.com) 等工具只是返回了成页的缩略图，用户必须逐一查看。
